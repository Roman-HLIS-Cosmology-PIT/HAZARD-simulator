...
class CosmicRaySimulation:
    ...
    def run_full_sim(cls,
        ...
        Returns:
            ...
            gcr_counts       : List[(species_name, count)]
        """
        heatmap_list = []
        streaks_list = []
        gcr_counts   = []

        # loop over species indices
        for idx in tqdm(range(len(cls.Z_list)),
                        desc="Running simulation for each species",
                        disable=not progress_bar):
            # instantiate with same kwargs + override species_index & grid_size
            sim = cls(species_index=idx,
                      grid_size=grid_size,
        ...
        return combined_heatmap, heatmap_list, streaks_list, gcr_counts

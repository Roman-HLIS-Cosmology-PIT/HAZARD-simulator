...
class ElectronWaveFunctionGRC:
    ...
    Electron Wave Function class implements the parameters and RK-4 method for the electron's
    ...
    def diff_eq(self, r, F, G):
        """Radial component of the Dirac Eqn at this radius, F, and G"""
...
def wavefunc_renorm(Z, n, k, bound, h_found, Nr):
    ...
    if bound:
        fout_prime = (data_Fs[:-1] * (overflow_fix_constant) ** (delta_K / 2) - data_Fs[1:]) / (
            data_rs[:-1] - data_rs[1:]
        )
        ...
        temp_data_rs, temp_data_Fs, temp_data_Gs, K_count = instance2.RK_4(
            r_max, data_rs[rmatch_index], Nr - rmatch_index
        )
        ...
        # print(data['Fs_int_out'][rmatch_index]/data['Fs_int_out'][rmatch_index+1]
        #   *temp_data_Fs[-2]/temp_data_Fs[-1])
        # print(temp_data_Fs)
        # print(K_count)
...
if __name__ == "__main__":
    ...
    if find_h == "True":
        ...
        with open(path_out, "a") as f:
            f.write(
                str(n) + "\t" + str(k) + "\t" + str(data["h"]) + "\t" + str(data["h"] / 1.60218e-19) + "\n"
            )

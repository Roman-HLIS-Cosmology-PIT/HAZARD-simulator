...
class CosmicRaySimulation:
    ...
    def __init__(
        ...
        material_A=A_mean,
        I0=I_value_MeV,
        material_density=HgCdTe_density,
        X0=X0_cm,
        color_list=color_list,
        date=2018 + frac_amounts[6],
        ...
        date : float, default=2018 + frac_amounts[6]
    ...
    def run_full_sim(
        ...
        **sim_kwargs
        ...
        return (
            ...
            streaks_list,
            gcr_counts,
        )  # must fix combined_heatmap eventually
    ...
    def beta(Ekin, mass):
        """
        Compute the dimensionless relativistic velocity β = v/c for a particle.

        Parameters
        ----------
        Ekin : float
            Kinetic energy of the particle (in the same energy units as `mass`, typically MeV).
        mass : float
            Rest mass energy of the particle (e.g., MeV).

        Returns
        -------
        float
            The dimensionless velocity β = p / E_total, where
            p = sqrt(E_total² − mass²) and E_total = Ekin + mass.
        """
    ...
    def save_sim(self, heatmap, streaks_list, gcr_counts, filename):
        ...
        species_arr = np.array(
            [str(s) for s in species_names_list], dtype=h5py.string_dtype(encoding="utf-8")
        )
    ...
    def rigidity(self, energy, A, Z, m):
        """
        Particle rigidity R = pc / (|Z|e) expressed as a function of kinetic energy.
    ...
    def log_rigidity_spectrum(self, alpha, beta, g, C, R, D, R0):
        ...
        beta : float
            Particle speed in units of c (0–1).
        g : float
...
#NEW DELTA RAY POPULATION CODE BELOW
    ...
    def _Eproj_min_from_electron_E(self, Te_MeV, Mproj_MeV):
        ...
        with kinetic energy Te_MeV, using your board derivation:
            E_z,min = M * [ (E/(2 m_e)) / ( sqrt(1 + E/(2 m_e)) + 1 ) ]
        Works with scalars or numpy arrays.
        """
        x = np.asarray(Te_MeV, dtype=float) / (2.0 * self.me)      # Te/(2 m_e)
        return Mproj_MeV * ( x / (np.sqrt(1.0 + x) + 1.0) )


    def _primary_flux_per_species(self, species_idx, bin_edges_eV):
        """F_Z(E) per eV (same ISO machinery as run_sim, but flux density only)."""
        E_mid = 0.5 * (bin_edges_eV[:-1] + bin_edges_eV[1:])
        dE    = np.diff(bin_edges_eV)

        A  = self.A_list[species_idx]
        Zp = self.Z_list[species_idx]
        m  = self.m_list[species_idx]  # eV
        phi = np.zeros_like(E_mid, dtype=float)

        for i, Eev in enumerate(E_mid):
            R   = self.rigidity(Eev, A, Zp, m)
            R0  = self.compute_R0(self.date, R)
            bet = self.relative_velocity(Eev, m)
            g   = self.gamma_func(R, species_idx)
            D   = self.Delta(Zp, bet, R, R0)
            ln_phi = self.log_rigidity_spectrum(self.alpha_list[species_idx], bet, g,
                                                self.C_list[species_idx], R, D, R0)
            val = np.exp(ln_phi)
            phi[i] = 0.0 if (not np.isfinite(val) or val <= 0) else val
        return E_mid, dE, phi  # centers (eV), widths (eV), flux density per eV


    def _Wmax_primary(self, Ekin_MeV, M_MeV):
        """W_max (MeV) for a primary with kinetic energy Ekin_MeV and rest mass M_MeV."""
        beta_val  = self.beta(Ekin_MeV, M_MeV)
    ...
    def compute_secondary_electron_flux(
        ...
        """
        # Use/extend your standard ISO binning
        ...
        if extend_low_electron_E and E_e_min_eV < e_edges[0]:
            e_edges = np.concatenate([[E_e_min_eV], e_edges])
        ...
        for sidx, zcharge in enumerate(self.Z_list):
            ...
            Ep_mid_eV, dEp_eV, FZ = self._primary_flux_per_species(sidx, e_edges)
            # Kinematics for this species
            M_MeV = self.m_list[sidx] * 1e-6
            Ep_mid_MeV = Ep_mid_eV * 1e-6

            # β_z(E'_z) and Wmax(E'_z)
            beta_p = np.array([self.beta(Ep, M_MeV) for Ep in Ep_mid_MeV])
        ...
        for iE, Te in enumerate(E_e_mid_MeV):
            ...
            Ezmin = self._Eproj_min_from_electron_E(Te, M_MeV)
            ...
            contrib = np.sum(FZ_i * K * (dEp * 1e-6))  # ΔE′ in MeV
            lnLambda = self._lnLambda_electron(Te)
            if lnLambda > 0.0:
                F_e[iE] += contrib / lnLambda

            # Output is a flux density per eV on e_edges
        return e_edges, E_e_mid_eV, F_e
...
#END NEW DELTA RAY CODE
    ...
    def propagate_delta_rays(self, heatmap, x, y, z, theta, phi, init_en, PID, streaks):
        ...
        while current_energy > 0:
            ...
            )  # rotation matrix: 1st column is "North" direction, 2nd column is "East"
...
#NEW DELTA RAY POPULATION CODE
        if idx == 0:  # electron channel (Z = -1)
            ...
            dE_e = np.diff(e_edges)
            extra_means = np.nan_to_num(F_e * dE_e * self.dOmega * self.dt * self.dA)
            ...
            E_base = num_part_table['Bin Center Energy (eV)'].values
            E_extra = e_centers
